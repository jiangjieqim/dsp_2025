...
{h2}icpdf/STC89C51RC-RD.pdf{!h2}  
{h2}icpdf/STC89CXX.pdf{!h2}  
{h2}icpdf/STM32F103X6.pdf{!h2}  
{h2}icpdf/89C51.pdf{!h2}  
...
[74HC373](#74HC373)*8位串行输入,并行输出的位移缓存器*  
...
按照以下步骤操作，就能找到并开启这两个关键选项：

在项目窗口找到文件：在 Keil 界面的左侧 `Project Workspace` 窗口中，找到你的 C 文件（比如 test_project1.c）。

右键打开文件属性：右键点击这个 C 文件名，在弹出的菜单中选择 `Options for File 'test_project1.c'`。

勾选两个关键选项：在弹出的对话框中，你会看到两个复选框，需要将它们的状态变成黑色✔（默认是灰色，需要点击一下）：

`Generate Assembler SRC File`

`Assemble SRC File`

（重要）添加库文件：勾选确认后，还需要在工程中加入一个库文件，否则链接时会报错。

打开 Keil 安装目录下的 C51\LIB 文件夹（例如 C:\Keil_v5\C51\LIB）。

根据你的编译模式（默认通常是 Small），找到 `C51S.LIB` 文件。

将它复制到你的工程文件夹中，然后在 Keil 里像添加 C 文件一样，
右键 Source Group 选择 Add Existing Files to Group…，把这个 .LIB 文件添加进工程。
...
5.  `波特率可以选择比较低的,越低越可靠`最低波特率和最高波特率都可以设置为1200   
...
比如设置波特率为 9600，就代表每秒传输 9600 个二进制位

`Q 如何配置89C51开发环境?`  

1. `github\dsp_2025\software_steup\keil+uvision4破解版.zip`  
2. `github\dsp_2025\software_steup\STC-isp\USB to UART Driver\CH340_CH341`  
3. 发送`github\dsp_2025\software_steup\STC-isp\stc-isp-15xx-v6.67B.exe`快捷方式到桌面   

`Q ISP指的是什么?`  

ISP 的全称是 In-System Programming，中文意为“在系统编程”。

它的核心特点就是不需要把芯片从电路板上拆下来，也不需要专用的编程器，直接通过串口、USB等接口，在电路板上就能对单片机进行程序烧录或升级。

针对你之前问的 STC-ISP 软件，这里的 ISP 指的就是这项技术——STC 单片机支持通过串口直接在系统内编程。

`Q 调低波特率之所以能提高单片机下载成功率，核心原因?`  

调低波特率之所以能提高单片机下载成功率，核心原因在于：低速通信对信号干扰、时钟误差和设备时序的要求都更低，整个系统更容易满足通信的基本条件。

1. 降低对信号质量的要求（抗干扰能力变强）
高速信号更“脆弱”：高波特率下，每个比特的持续时间非常短（115200bps时约为8.7微秒）。如果通信线路接触不良、导线过长、受到电机等电磁干扰，信号波形就容易变形。

低速信号更“强壮”：当波特率降到9600bps时，一个比特持续约104微秒。即便信号有些毛刺或轻微变形，接收方在采样时也能大概率读到正确的电平。低速就像把话说得慢而清晰，即使环境嘈杂也能听清。

2. 宽容时钟误差（匹配要求降低）
双方时钟永远有误差：单片机的晶振不是绝对精确，USB转串口芯片的内部时钟也有偏差。双方时钟频率不可能完全一致。

误差会累积：串口通信是逐位采样的，波特率越高，采样速度越快，同样的时钟误差在单位时间内累积的偏差就越大。当累积偏差超过允许范围（通常为3%-5%），就可能读错位，导致数据错乱。

低速累积慢：低速下，同样误差造成的位偏移量更小，仍在接收端的容忍范围内。就像两辆速度略有差异的车，行驶距离越长距离拉得越大；低速相当于短跑，误差来不及放大就到终点了。

3. 适应“冷启动”和电源不稳定
冷启动干扰：STC单片机下载要断电再上电。重新上电瞬间，电源有起伏波动，可能影响ISP引导程序立即以高波特率稳定运行。

低速更稳定：以较低波特率运行引导程序，对电源的瞬时要求更低，更容易成功进入下载状态。

4. 兼容老旧的硬件和驱动
老式USB转串口芯片性能有限：一些廉价或较早的USB转串口模块（如PL2303早期版本），在高波特率下数据容易出错。

虚拟串口不稳定：操作系统或驱动在高波特率下处理不好，模拟出的串口也会丢数据。

低速通用性强：2400/9600是所有串口设备都支持的“安全速度”，几乎不会出现兼容性问题。
...
`Q 为什么在proteus中没有VCC引脚?`  
因为proteus默认接上电源了，所以隐藏了就不必管它了  

`Q 为什么延时实现用多个u_delay255MS函数而不使用for循环实现?`  
因为for也是会占用时钟周期的,而函数压栈消耗的时钟周期的误差时间小  

`Proteus-> Schematic Capture:No Libraries Found`  
...
调用函数：LCALL 或 ACALL 指令 → 2µs

函数返回：RET 指令 → 2µs

保存/恢复寄存器（如 push / pop，取决于函数是否使用寄存器）→ 每条 1µs，通常加 2–5µs
```
`******************************`
```
void delay_nop(void) {
...
`Q 什么是额定电流`  
额定电流是指电气设备在额定工作条件（如额定电压、额定功率、规定环境温度等）下，能够长期、安全、稳定运行所允许通过的最大电流。

简单理解就是：设备正常工作时，给它通多大的电流是安全的。

`Q 8051 P0作为I/O使用的时候为什么要加上拉电阻?`  

`P0 端口内部是漏极开路（Open-Drain）结构，没有内部上拉电阻`  
P0 作为普通 I/O 口时，内部上拉管被切断，只剩下一个只能拉到 GND 的下拉管，所以输出 1 时必须靠外部上拉电阻把电平拉到 VCC。
...
`Q Simulation is not running in real time due to excessive CPU load. 0.0128880215s模拟器的警告`  

这个警告的意思是：仿真器因为CPU负载过高，无法以实时速度运行。你看到的时间 0.0128880215s 是仿真器实际模拟这段代码所花费的时间，而不是真实世界的时间。
...
_nop_();
```
这样MCU会以最高速度翻转IO口（可能达到几百kHz甚至MHz级别），导致仿真器跟不上。

`Q 89c51为什么分频`  

原始8051设计于1980年代，当时半导体工艺和逻辑门延迟较大，需要较长的机器周期来保证内部各模块（特别是ROM/RAM访问）的稳定时序。
...
* (1) TMOD & 0xF0
        ...
        按位与操作后：保留高 4 位（定时器 1 的设置），低 4 位清零。

* (2) | 0x01

        0x01 = 0000 0001

        按位或操作后：低 4 位强制设为 0001（高 4 位不变）。
...
* (3) 最终效果  
        ...
        0	1	        模式 1：16 位定时器
        1	0	        模式 2：8 位自动重装
        1	1	        模式 3：两个独立 8 位定时器（仅定时器 0）

        定时器 1：模式不变（原来是哪种模式还是哪种模式）
        定时器 0：设置为 模式 1（16 位定时器/计数器）
...
下面是它的内存布局示意图，可以看到其在 80C51 基础上增加了新的区域：
...
`低电流的物理基础`：作为微型电机，R300C的线圈绕组细、磁铁小，运转时产生的反电动势（抵抗电流的电压）小，所以只需很小的电流就能维持转动。

宽电压的实现方式：它的内部结构简单，没有复杂的电子控制电路，因此对输入电压不敏感，只要在允许范围内都能工作。

`电压是驱动电机运转的根本条件，它必须达到一个最低值，电机才能启动。`
```
提供启动门槛：R300C的起步电压是1.5V。只有给它施加至少1.5V的电压，内部才会有足够的电流来产生启动转矩。

控制转速：一旦启动，电机的转速就和电压成正比。电压越高，转得越快。例如，R300C在3V电压下约3500 RPM，在6V电压下能达到7000 RPM。它的额定工作电压是3V，建议在3-5V下运行以获得最佳表现

```

`电流：取决于“干多少活”`  
电流不是由电压单独决定的，它的大小主要取决于电机所带的负载（机械阻力）。

空载时电流极小：当电机只是空转，没有带任何东西时，它只需要克服自身的摩擦，这时消耗的电流非常低。R300C空载电流通常小于0.1A，典型值仅为0.02A (20mA)。

负载越大，电流越大：当给电机轴上加上负载（比如风扇叶片、车轮）时，电机需要输出更大的力（转矩）来带动它们。输出转矩与电流成正比，因此负载越重，电流就越大。

额定电流是安全参考：R300C的额定电流是0.05A，这是它在额定功率下工作的参考值。实际工作中，只要不超过其承受极限，电流会随负载自动变化。

`空载电流`  
简单来说，就是电机不带任何负载（轴上不挂任何东西）时，为了维持自身空转所消耗的最小电流。
